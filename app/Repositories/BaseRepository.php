...
abstract class BaseRepository implements IRepository
{
     ...
     * @return Builder
     */
    function search(BasicFilter $filter = null, bool $onlyActive = true, array $withs = [], bool $onlyTrashed = false): Builder
    {
        if (is_null($filter))
        ...
        # Only trashed
        if ($onlyTrashed == true) return $model::query()->onlyTrashed();
        else $query = $model::query();
    ...
    /**
     * Try to force-delete a model record
     * @param mixed $id
     * @param string $idColumnName
     * @return bool
     */
    function forceDelete(mixed $id, string $idColumnName = 'id'): bool
    {
        $model_class = $this->getRepositoryModelClass();
        $model = new $model_class();
        return $model::withTrashed()->where($idColumnName, $id)->forceDelete();
    }
     ...
     * @return bool
     */
    function restore(mixed $id, MetaInfo $meta = null, string $idColumnName = 'id'): bool
    {
        $model_class = $this->getRepositoryModelClass();
