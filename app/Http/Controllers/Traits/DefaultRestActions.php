...
trait DefaultRestActions
{
    ...
    public function search(Request $request): Response
    {
        ...
        $requireToTranslate = false;
        $withs = $payload['withs'] ?? [];
        $onlyTrashed = $payload['only_trashed'] ?? false;
        if ($this->isTranslatable() && $meta->lang !== SupportedLanguages::DEFAULT_LOCALE) {
            $requireToTranslate = true;
        ...
        $result = $this->getService()->search($payload, paging: $paging, withs: $withs, onlyTrashed: $onlyTrashed);
     ...
     * @return Response
     */
    public function forceDelete(DefaultIdSlugRequest $request): mixed
    {
        $request = $this->validateRequest($request, 'forceDelete');
        $ret = $this->getService()->forceDelete($request->input('id'));
        return $this->getResponseHandler()->report($ret);
    }
     ...
     * @return Response
     */
    public function restore(DefaultIdSlugRequest $request): mixed
    {
        $request = $this->validateRequest($request, 'restore');
        $ret = $this->getService()->restore($request->input('id'), commandMetaInfo: $this->getCurrentMetaInfo());
        return $this->getResponseHandler()->report($ret);
    }
}
