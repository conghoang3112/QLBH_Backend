...
class RoleService extends \App\Services\BaseService implements IRoleService
{
    ...
    public function search(array $rawConditions, PaginationInfo &$paging = null, array $withs = [], bool $onlyTrashed = false): Collection
    {
        try {
            ...
            $query = $this->roleRepos->search($basicFilter, onlyTrashed: $onlyTrashed);
    ...
    public function forceDelete(int $id): bool
    {
        DB::beginTransaction();
        try {
            $result = $this->roleRepos->forceDelete(id: $id);
            DB::commit();
            return $result;
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public function restore(int $id, MetaInfo $commandMetaInfo = null): bool
    {
        DB::beginTransaction();
        try {
            $result =  $this->roleRepos->restore(id: $id, meta: $commandMetaInfo);
            DB::commit();
            return $result;
        } catch (\Exception $ex) {
            DB::rollBack();
