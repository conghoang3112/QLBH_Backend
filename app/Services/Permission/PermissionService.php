...
class PermissionService extends \App\Services\BaseService implements IPermissionService
{
     ...
     * @throws Throwable
     */
    public function search(array $rawConditions, PaginationInfo &$paging = null, array $withs = [], bool $onlyTrashed = false): Collection
    {
        try {
            $basicFilter = new BasicFilter();
            $query = $this->permissionRepos->search($basicFilter, onlyTrashed: $onlyTrashed);
    ...
    public function delete(int $id, bool $softDelete = true, MetaInfo $commandMetaInfo = null): bool
    {
        ...
        try {
            ...
            $result = $this->permissionRepos->delete(id: $record->id, soft: $softDelete, meta: $commandMetaInfo);
            DB::commit();
            return $result;
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    /**
     * Force-delete item from resources
     *
     * @param int $id
     * @param MetaInfo|null $commandMetaInfo
     * @return bool
    ...
    public function forceDelete(int $id): bool
    {
        DB::beginTransaction();
        try {
            $result = $this->permissionRepos->forceDelete(id: $id);
            DB::commit();
            return $result;
        } catch (\Exception $ex) {
            DB::rollBack();
            throw new ActionFailException(
                message: 'force delete: ' . json_encode(['id' => $id]),
                previous: $ex
            );
        }
    }
     ...
     * @throws ActionFailException
     */
    public function restore(int $id, MetaInfo $commandMetaInfo = null): bool
    {
        DB::beginTransaction();
        try {
            $result =  $this->permissionRepos->restore(id: $id, meta: $commandMetaInfo);
            DB::commit();
            return $result;
        } catch (\Exception $ex) {
            DB::rollBack();
            throw new ActionFailException(
                message: 'restore: ' . json_encode(['id' => $id]),
                previous: $ex
            );
        }
    }
